...
export async function POST(request: Request) {
  try {
    ...
    if (["running"].includes(task.status)) return NextResponse.json({ error: "This task is already running." }, { status: 409 });
    ...
    const [{ data: company, error: companyError }, { data: knowledge, error: knowledgeError }] = await Promise.all([
    ...
    const geminiResponse = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${GEMINI_MODEL}:generateContent`, {
      ...
      body: JSON.stringify({
        system_instruction: { parts: [{ text: `You are ${agent.name}, the ${agent.role} at ${company.name}. ${agent.system_prompt ?? "Complete the assigned task carefully and practically."} Work toward this company goal: ${company.goal}. Produce a useful work output for a human to review. Do not claim external actions were taken. If information is missing, say what is missing. Use the supplied company knowledge when relevant and do not invent company policy.` }] },
    ...
    if (!geminiResponse.ok) {
      ...
      console.error("Gemini agent error", geminiResponse.status, details);
    ...
    return NextResponse.json({ task: updatedTask, sources: matches.map((match) => match.title).filter((title, index, all) => all.indexOf(title) === index) });
  } catch (error) {
